public class ToTest {
	...
	private static int getSum_member(int a, int b, int c) {
		...
		assert p3Dx.sum() != a + b;
		return p3Dx.sum(); // @Replace should replace wrong student code "return p3Dx.sum();" with expected code "return p3D.sum();"
	}
	...
	private static int getSum_member_anonymous_inner(int a, int b, int c, int d) {
		...
		}.getSum_member_anonymous_inner(a, a, c, d); // @Replace should replace wrong student code "(a, a, c, d)" with expected code "(a, b, c, d)"
	}
}
