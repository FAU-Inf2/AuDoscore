...
public class ToTest {
	...
	private static List<Integer> getSome() { // intentionally overloaded!
		...
		odds.removeIf(x -> x % 2 == 0); // @Replace should replace wrong student code "removeFirst" with expected code "x -> x % 2 == 0"
		odds.removeIf(x -> x == 5); // @Replace should replace wrong student code "removeLast" with expected code "x == 5"
		return odds;
	}
	...
	private List<Integer> getSome(int v) { // intentionally overloaded!
		...
		List<Integer> odds = new ArrayList<>(DIGITS);
		odds.removeIf(x -> x % 2 != 0); // @Replace should replace wrong student code "removeFirst" with expected code "x -> x % 2 != 0"
		odds.removeIf(x -> x == 6); // @Replace should replace wrong student code "removeLast" with expected code "x == 6"
		return odds;
	}
}
