...
public class ToTest {
	...
	private static List<Integer> getSome() { // intentionally overloaded!
		...
		odds.removeFirst(); // @Replace should replace wrong student code "x -> x % 2 == 0" with expected code "removeFirst"
		odds.removeLast(); // @Replace should replace wrong student code "x == 5" with expected code "removeLast"
		return odds;
	}
	...
	private List<Integer> getSome(int v) { // intentionally overloaded!
		...
		List<Integer> odds = new ArrayList<>(DIGITS);
		odds.removeFirst(); // @Replace should replace wrong student code "x -> x % 2 != 0" with expected code "removeFirst"
		odds.removeLast(); // @Replace should replace wrong student code "x == 6" with expected code "removeLast"
		return odds;
	}
}
